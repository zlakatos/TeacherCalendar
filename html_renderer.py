...
with st.sidebar.expander("🌟 **Különleges hetek és napok**", expanded=False):
    with st.expander("🟢 Zöld hét beállítása", expanded=False):
        green_start = st.date_input("Kezdeti nap", value=parse_d(saved_data.get("green_start", "2026-09-07")), key="green_start")
    ...
    with st.expander("🟢 Iskola másként hét beállítása", expanded=False):
        other_school_start = st.date_input("Kezdeti nap", value=parse_d(saved_data.get("other_school_start", "2026-10-19")), key="other_start")
    ...
    with st.expander("🟧 Karácsonyi vásár beállítása", expanded=False):
        christmas_market_date = st.date_input("Vásár napja", value=parse_d(saved_data.get("christmas_market_date", "2026-12-18")), key="xmas_market")

    with st.expander("🟪 Ballagás beállítása", expanded=False):
        graduation_date = st.date_input("Ballagás napja", value=parse_d(saved_data.get("graduation_date", "2027-06-04")), key="graduation")
...
calendar_html = build_calendar_html(
    ...
    saved_data.get("half_filled_classes", []), saved_data["classes"], 
